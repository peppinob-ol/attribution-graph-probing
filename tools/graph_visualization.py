"""
Vendored copy of demos/graph_visualization.py from circuit-tracer.

Source: https://github.com/safety-research/circuit-tracer
Tag/commit: v0.3.1 (e09b5f3) -- the same version that produced the existing
graph.json files under output/.
License: MIT (matches the upstream repository).

The upstream module is shipped only inside the demos/ folder of the repo and
not as part of the installable package. We vendor it here so the SVG circuit
visualization (Supernode / InterventionGraph / create_graph_visualization) can
be used from this codebase without depending on the demos/ folder.

Only the file header and these top-level comments were added. The rest of
the file is verbatim from upstream.
"""
