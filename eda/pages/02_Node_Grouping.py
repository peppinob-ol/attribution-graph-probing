...
try:
    ...
    st.success(f"✅ CSV loaded: {csv_name} - {len(df)} rows, {df['feature_key'].nunique()} unique features")
except Exception as e:
...
if json_to_use:
    try:
        ...
        st.success(f"✅ JSON activations loaded: {json_name} - {n_prompts} prompts")
    except Exception as e:
...
if nodes_json_to_use:
    try:
        ...
        st.success(f"✅ Selected Nodes JSON loaded: {n_features} features, {n_nodes} nodes")
    except Exception as e:
...
if 'df_named' in st.session_state:
    ...
    # Upload button
    if st.button("🚀 Upload to Neuronpedia", disabled=not (api_key and graph_json_available)):
        if not api_key:
            ...
            try:
                ...
                if 'url' in result:
                    st.markdown(f"🔗 [View on Neuronpedia]({result['url']})")
