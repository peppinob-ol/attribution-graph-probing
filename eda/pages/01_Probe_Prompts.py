...
# Auto-load from pipeline if available (do this first, before showing UI)
if 'pipeline_graph_json' in st.session_state and 'graph_json' not in st.session_state:
    # Load from pipeline state
    st.session_state['graph_json'] = st.session_state['pipeline_graph_json']['data']
    st.session_state['graph_source'] = st.session_state['pipeline_graph_json']['filename']

# Check if graph was auto-loaded from pipeline
# Compare filenames (basename only, ignoring paths)
graph_auto_loaded = False
if 'graph_json' in st.session_state and 'pipeline_graph_json' in st.session_state:
    pipeline_filename = st.session_state['pipeline_graph_json']['filename']
    current_source = st.session_state.get('graph_source', '')
    
    # Extract basename from both (in case current_source has path)
    from pathlib import Path
    pipeline_basename = Path(pipeline_filename).name if pipeline_filename else ''
    current_basename = Path(current_source).name if current_source else ''
    
    # Consider auto-loaded if basenames match or if current_source matches pipeline_filename exactly
    graph_auto_loaded = (pipeline_basename == current_basename) or (current_source == pipeline_filename)

if graph_auto_loaded:
    # Show info and collapse load section
    filename = st.session_state['pipeline_graph_json']['filename']
    st.success(f"✅ Graph auto-loaded from Step 1: `{filename}`")
    
    # Option to select a different graph
    with st.expander("📁 Load a different graph file", expanded=False):
    ...
    # Normal display: no auto-load, show load section directly
    # List available JSON files
    output_dir = parent_dir / "output" / "graph_data"
    json_files = []
    if output_dir.exists():
...
if graph_json:
    ...
    # Auto-load from pipeline if available (do this first, before showing UI)
    if 'pipeline_selected_nodes' in st.session_state and 'filtered_features' not in st.session_state:
        try:
            # Load selected nodes data from pipeline
            selected_nodes_data = st.session_state['pipeline_selected_nodes']['data']
            
            # Extract features (same logic as tab_load)
            if isinstance(selected_nodes_data, dict) and "features" in selected_nodes_data:
                features_json = selected_nodes_data["features"]
            elif isinstance(selected_nodes_data, list):
                features_json = selected_nodes_data
            else:
                raise ValueError("Unrecognized format")
            
            # Convert to features_in_graph format
            features_in_graph = []
            nodes = graph_json.get("nodes", [])
            ...
            st.warning(f"⚠️ Could not auto-load features from pipeline: {e}")
    
    # Check if features were auto-loaded from pipeline
    features_auto_loaded = ('filtered_features' in st.session_state and 
                           'pipeline_selected_nodes' in st.session_state)
    
    if features_auto_loaded:
        # Show info and collapse tabs
        n_features = len(st.session_state['filtered_features'])
        filename = st.session_state['pipeline_selected_nodes']['filename']
        st.success(f"✅ Features auto-loaded from Step 1: `{filename}` ({n_features} features)")
        
        # Option to select a different subset
        with st.expander("📁 Load a different feature subset", expanded=False):
            tab_load, tab_all, tab_export = st.tabs(["Load Subset", "Use All", "Export Subset"])
    else:
        # Normal display: no auto-load, show tabs directly
        tab_load, tab_all, tab_export = st.tabs(["Load Subset", "Use All", "Export Subset"])
        ...
        **Expected format:**
        ...
        if uploaded_file is not None:
            try:
                ...
                st.session_state['pipeline_activations_json'] = {
            ...
            if 'activations_uploaded_data' in st.session_state:
                ...
                try:
                    ...
                    if not verify_full_final.empty:
                        # Save to pipeline session_state for auto-loading in next steps
                        st.session_state['pipeline_analysis_csv'] = {
                            'data': verify_full_final,
                            'filename': f"st2_probe_metrics_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv",
                            'timestamp': datetime.now().isoformat()
                        }
