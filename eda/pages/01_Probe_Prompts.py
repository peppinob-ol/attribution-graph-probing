...
st.write("""
Load the JSON file of an attribution graph generated by Neuronpedia.
You can get it from:
- Locally saved file (e.g.: `output/graph_data/anthropological-circuit.json`)
- Neuronpedia API: `https://www.neuronpedia.org/api/graph/{modelId}/{slug}`
""")

# Loading tabs
tab_file, tab_url = st.tabs(["From File", "From URL/API"])
...
with tab_url:
    st.write("Load a graph directly from Neuronpedia API")
    ...
    with col1:
        api_model_id = st.text_input(
            "Model ID",
            value="gemma-2-2b",
            help="E.g.: gemma-2-2b, gpt2-small"
        )
    with col2:
        api_slug = st.text_input(
            "Graph Slug",
            value="",
            help="The graph slug (from Neuronpedia URL)"
        )
    
    if st.button("Load from API", type="primary"):
        if not api_slug:
            st.error("⚠️ Enter a valid slug")
        else:
            try:
                import requests
                ...
                st.session_state['graph_json'] = graph_json
                st.session_state['graph_source'] = graph_source
                st.success(f"✅ Graph loaded from API: {api_model_id}/{api_slug}")
                st.rerun()
...
if graph_json:
    ...
    st.write("""
    Load a JSON file with the list of features to analyze, or use all features from the graph.
    """)
...
if 'concepts' in st.session_state and st.session_state['concepts']:
    ...
    with col1:
        if st.button("Save Concepts as Prompts JSON"):
            ...
            # Use export_concepts_to_prompts function
            export_concepts_to_prompts(st.session_state['concepts'], str(output_prompts_path), verbose=False)
            st.success(f"✅ Prompts saved in: {output_prompts_path.relative_to(parent_dir)}")
            st.info("Format: `[{\"id\": \"probe_X\", \"text\": \"category: description is label\"}, ...]`")
                    ...
                    if not csv_path.exists():
                        ...
                        try:
                            ...
                            if act_df.empty:
                                ...
                                with st.expander("Data Verification Table (JSON + CSV)", expanded=False):
                                    st.caption("""
