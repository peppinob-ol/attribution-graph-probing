...
The appendix is organized to track the main paper: \S\ref{appx:pipeline}--\S\ref{appx:funcvocab} give a detailed reference for the pipeline and its inputs; \S\ref{appx:clustering} validates the four-role partition against the natural cluster geometry of the CPAS metric space; \S\ref{appx:specificity}--\S\ref{appx:fieldadd} back up the results of \S\ref{sec:results:primary} with per-domain numbers; \S\ref{appx:msearch}--\S\ref{appx:kl} back up the $M$-search and steering-strength discussion of \S\ref{sec:results:diagnostic:msearch}; \S\ref{appx:topkim} runs the per-pair influence-matched top-$K$ baseline across four in-scope domains, and \S\ref{appx:dallas_case} is the all-49-source Dallas case study with the human/ours headline and a fixed-$K$ saturation control; \S\ref{appx:crossprompt}--\S\ref{appx:scaffold} back up the cross-prompt and scaffold analyses of \S\ref{sec:results:diagnostic:layers} and \S\ref{sec:results:diagnostic:scaffold}; \S\ref{appx:regime} gives the logit-shift regime taxonomy used throughout; \S\ref{appx:case} contains worked case studies; \S\ref{appx:negative} discusses the weakest of our four domains (Paintings); \S\ref{appx:repro} is the reproducibility manifest; \S\ref{appx:glossary} is the glossary. Every appendix section is self-contained and cross-referenced from the main text.
...
\begin{figure}[h]
  ...
  \caption{\textsc{Sem-Dict} (fallback) --- \emph{Rhode Island / Warwick} entity, U.S.\ states domain, layer-3 feature 81381. The feature fires on every probe and behaves like a Rhode-Island detector, but its activation does not collapse onto a single surface token: it lights up both subwords of the multi-token state name (\texttt{rhode}, \texttt{island}) and re-fires on the bare \texttt{island} elsewhere in the batch. The fallback rule of \S\ref{appx:pipeline} is meant to capture exactly this regime---dictionary-like specificity at a layer too shallow for the strict criterion, with a peak that drifts across morphologically related tokens before the network has had room to collapse them onto a single representative.}
...
\begin{figure}[h]
  ...
  \caption{\textsc{Relationship} --- \emph{Delaware / Wilmington} entity, U.S.\ states domain, layer-7 feature 66851. Rather than concentrating on one token, the feature is active on a substantial fraction of every probe: a diffuse green band runs across each row, with several peaks spread between functional and semantic positions and no single token dominating the activation budget. This combination---low sparsity, no clear peak target---is the visual signature of \textsc{Relationship} features.}
...
\begin{figure}[h]
  ...
  \caption{\textsc{Say-X} --- \emph{Oracle DB} entity, products domain, layer-11 feature 9574. A late-layer feature whose only strongly-active position in each probe is the copula \texttt{is} immediately preceding the next-token prediction. Because every active position is a functional token, the $\pm 7$-token search of \S\ref{appx:funcvocab} resolves the actual semantic target by looking forward from the copula---in this case, into the answer slot.}
...
The decision tree of \cref{tab:decision-thresholds} carves the per-feature CPAS metric space (\texttt{peak\_consistency\_main}, \texttt{n\_distinct\_peaks}, \texttt{func\_vs\_sem\_pct}, \texttt{conf\_F}, \texttt{sparsity\_median}, layer) with a small number of axis-aligned thresholds and produces four functional roles. A natural concern is whether those four labels reflect dense regions of the underlying metric space or impose arbitrary cuts. We tested it directly by re-aggregating the per-feature metrics across all four datasets into a manifest of $\approx 8{,}000$ deduplicated $(\text{layer}, \text{feature})$ rows and running unsupervised clustering with several methods.
...
We standardized the six metrics and projected them with t-SNE per dataset (\cref{fig:tsne-per-dataset}), colouring each feature by its subtype rather than its top-level role---so that \textsc{Sem-Dict} is split into the strict \emph{Dictionary} and \emph{Dictionary (fallback)} buckets defined in \S\ref{appx:pipeline}. Across all four domains the categories occupy spatially coherent regions: \textsc{Relationship} forms a single low-sparsity arc, \textsc{Say-X} a tight late-layer cluster, and \textsc{Sem-Conc} occupies the layer-shallow midband. The \emph{Dictionary (fallback)} bucket consistently appears in multiple visually disconnected regions, an early hint that this branch of the decision tree sweeps up two or more genuinely different feature populations.

\begin{figure}[h]
  \centering
  \includegraphics[width=\textwidth]{figures/fig_tsne_per_dataset.pdf}
  \caption{Per-dataset t-SNE of the standardized 6-metric CPAS vector. Points are coloured by feature subtype (strict \emph{Dictionary}, \emph{Dictionary (fallback)}, \emph{Concept}, \emph{Relationship}, \emph{Say-X}; see \S\ref{appx:pipeline}). \textsc{Relationship} and \textsc{Say-X} form tight clusters; the \emph{Dictionary (fallback)} bucket appears in multiple disconnected regions in every domain.}
...
The matched-random control of \S\ref{appx:specificity} fixes the feature \emph{count} and per-layer histogram of the labeled intervention, but it does not control for the cumulative graph influence consumed by the intervention: a labeled pair whose features happen to land on the highest-influence nodes of the circuit has a structural advantage that has nothing to do with the concept-aligned grouping. We therefore add a second per-pair-matched control, \textsc{Top-$K$ Influence-Matched}, that holds the per-side cumulative \texttt{node\_influence} fixed at the labeled best-of value rather than the feature count.
...
\]
and analogously $K^{\mathrm{tgt}}$ on the target side. The intervention is a single bag (no field-additivity sub-selection), with $M_{\mathrm{ablate}}{=}-2$, $M_{\mathrm{amplify}}{=}20$, and the same outer adaptive-$M$ sweep enabled. The grouping universe used for the top-$K$ is the entity's \texttt{node\_grouping.csv} (i.e.\ the same set of steerable features the labeled pipeline classifies); scaffold/error rows ($f{=}-1$) and embedding/logit-only rows ($\text{layer}{<}0$) are excluded by construction. Per-pair budgets, achieved sums, and chosen $K$ values are written to \texttt{output/research/topk\_budgets\_<domain>.csv} and to each result JSON's \texttt{metadata.control.diagnostics}.

\subsection{Result}
\Cref{tab:topk-im} reports the head-to-head Hit\% comparison and an exact / continuity-corrected paired sign test (McNemar's contingency $b{=}{}$labeled-only-wins, $c{=}{}$top-$K$-only-wins). On every in-scope domain the labeled best-of beats the influence-matched top-$K$ on the same pairs at the same per-side influence budget, with paired $p$-values from $7.2\!\times\!10^{-5}$ (Paintings) to far below $10^{-30}$ (Books, USA). The companion 4-panel figure (\cref{fig:topk-im-4domains}) plots each domain's Hit\% gap against the per-condition mean number of amplified features, confirming that the labeled bag uses substantially \emph{more} features than top-$K$ at the same per-pair influence budget, while still scoring higher on the redirection metric.

\input{tables/T_topk_influence_matched}

\begin{figure}[h]
\centering
\includegraphics[width=\linewidth]{figures/fig_topk_im_4domains.pdf}
\caption{Per-pair influence-matched top-$K$-by-influence (green) vs labeled best-of (red) across the four in-scope domains. By construction the per-pair target-side influence consumed by the two markers is the same (annotation in each panel; mean overshoot ${\sim}1.4\%$ from the smallest-prefix rule); the $y$-gap is the apples-to-apples Hit\% drop, and the $x$-gap shows how many more amplification features the labeled bag uses to achieve it. Sounds is excluded ($N{=}30$, weakest signal, structural confounds; \S\ref{appx:negative}).}
\label{fig:topk-im-4domains}
\end{figure}

The per-domain feature-count contrast is informative on its own: labeled bags include roughly 80--120 amplification features per pair, with mass spread across all three semantic supernodes (\S\ref{appx:fieldadd}); the budget-matched top-$K$ uses 13--45 features and concentrates them on the highest-influence nodes. Two readings are consistent with this gap and with the source-suppression rates we observe (\S\ref{sec:results:diagnostic:fields}): (i) a substantial fraction of the labeled features are low-influence ``scaffold'' nodes that nevertheless gate the answer circuit; (ii) the highest-influence nodes per the attribution graph are the \emph{loudest} features at the prompt's first token, but not the most informative for redirecting the model to a specific target answer (cf.\ \S\ref{appx:msearch}'s top-$k$-rescue null on Products). We do not separate (i) and (ii) within this experiment.

% --------------------------------------------------------------------
\section{USA Dallas Case Study (All 49 Sources)}
\label{appx:dallas_case}

The Dallas/Austin attribution graph is the only target circuit in our datasets that has a publicly-released human-curated subgraph (the Anthropic Neuronpedia graph \texttt{gemma-fact-dallas-austin}). We use this single-circuit asymmetry to run a focused case study: hold the target circuit fixed at \texttt{texas\_dallas}, run every non-Dallas USA state (49 sources) as the source side, and vary the target-side feature \emph{bag} across four conditions: human curation, our auto pipeline, an unlabeled top-$K$-by-influence control, and a label-shuffled floor control. The headline comparison is ours-vs-human; the top-$K$ family is a control that asks whether pure influence ranking can substitute for either, and the shuffled-labels condition is a sanity floor.
...
The source side (the 49 source states' ablations) is the canonical auto Dallas-target grouping in every condition --- on average $1{,}253$ features per state --- so the only quantity that varies across rows is the \emph{target bag}. The source-side bag is not held to any matching constraint, just held \emph{constant} across conditions (asymmetric design). Hit metric is the first-subword target token (`Austin') appearing in the steered continuation. Source hit-rate is the fraction of the 49 source states for which at least one cell hits.
...
\begin{figure}[h]
\centering
\includegraphics[width=0.85\linewidth]{figures/fig_topk_saturation_full50.pdf}
\caption{Successful swaps (\%) vs mean cumulative influence per swap, all 49 USA non-Dallas states swapping into Dallas/Austin. Red square = human curation (22 features, 5 supernodes); blue diamond = our auto pipeline (full Dallas grouping); green circles = fair top-$K$-by-\texttt{node\_influence} single-bag controls at $K \in \{21, 100, 200\}$ (top-10 omitted as redundant); grey X = random label-shuffled floor control. Both labeled methods reach $\sim 80\%$ successful swaps; the top-$K$ family saturates near $10\%$ at $1.09\times$--$2.93\times$ ours's per-swap influence.}
\label{fig:topk-saturation-full50}
\end{figure}
...
\begin{itemize}\itemsep0pt
\item Target bag: top-$K$ Dallas features by max \texttt{node\_influence} over the canonical Dallas grouping universe (\texttt{auto\_top<K>\_dallas/texas\_dallas/02 Node Grouping/node\_grouping.csv}). Source bag: canonical auto source's full grouping ($\sim 1{,}253$ features per state on average, no filter, no field-additivity).
...
\item Hit metric: first-subword target token (`Austin') in the steered continuation. Source hit-rate: fraction of the 49 source states with at least one hit.
...
\begin{figure*}[!htbp]
  ...
  \caption{La Grande Jatte $\to$ Water Lilies (\texttt{painter+first\_name} variant). The orange ``Claude'' curve barely peaks at $\sim 6\%$; ``Georges'' still dominates the trajectory and the steered continuation actually re-emits ``Georges Seurat''. A Hit in name only -- the position-0 logit prefers Claude but the autoregressive decoder reverts to the source painter. vsMax $+9.375$; the only Hit in the full Paintings batch ($1/90$ pairs), and the baseline rank of ``Claude'' was already~$3$. Illustrates that Paintings can be redirected only when baseline conditions are already favourable.}
...
\item[Completeness (Neuronpedia)] Fraction of incoming edges to all nodes of the subgraph that originate from \emph{grouped} features, weighted by influence on the output. Pipeline mean $0.89$ over $88$ entities across $4$ domains (\S\ref{sec:method:compression}).
...
\item[Position~0] The first generated token's logits, i.e.\ the model's output distribution immediately after consuming the prompt, before any autoregressive sampling. The default position at which regimes (\S\ref{appx:regime}) and KL are evaluated; complementary to the best-over-trajectory variant of vsMax (\S\ref{appx:pos0}).
