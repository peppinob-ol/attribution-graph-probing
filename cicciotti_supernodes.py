...
class CicciottiSupernodeBuilder:
    ...
    def select_semantic_seeds(self) -> List[Dict]:
        """
        Step 1: Selezione seed dai semantic_anchors
        
        Criteri: 
        - Massima label_affinity
        - Consistenza cross-prompt
        - Diversità per layer e peak_token
        """
        print(f"\n🌱 SEED SELECTION dai Semantic Anchors")
        print("=" * 50)
        
        semantic_anchors = self.archetypes.get('semantic_anchors', [])
        print(f"📊 Candidati seed disponibili: {len(semantic_anchors)}")
        
        # Ordina per qualità narrativa
        scored_seeds = []
        for anchor in semantic_anchors:
            feature_key = anchor['feature_key']
            personality = anchor['personality']
            
            # Score narrativo: affinità label + consistenza - deviazione
            narrative_score = (
                personality['label_affinity'] * 2 +  # Peso doppio per affinità label
                personality['consistency_score'] - 
                personality.get('consistency_std', 0)
            )
        ...
        # Ordina per score e diversifica
    ...
    def grow_cicciotto_supernode(self, seed: Dict, global_used_features: set = None) -> Dict:
        ...
        cicciotto = {
            ...
            'total_narrative_score': seed['narrative_score'],
        ...
        for iteration in range(max_iterations):
            ...
            if best_candidate and best_compatibility > 0.5:
                ...
                cicciotto['total_narrative_score'] += (
                    best_candidate['personality']['consistency_score'] * best_compatibility
                )
    ...
    def compute_narrative_compatibility(self, cicciotto: Dict, candidate: Dict) -> float:
        ...
        # Factor 3: Consistency compatibility
        consistency_diff = abs(
            seed_personality['consistency_score'] - 
            candidate_personality['consistency_score']
        )
    ...
    def compute_supernode_coherence(self, cicciotto: Dict) -> float:
        ...
        for member_key in cicciotto['members']:
            ...
            consistencies.append(personality['consistency_score'])
