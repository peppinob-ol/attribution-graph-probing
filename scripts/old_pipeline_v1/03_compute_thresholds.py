...
def compute_robust_thresholds():
    ...
    print(f"\nTau_inf (logit influence threshold):")
    ...
    print(f"\nTau_aff (max affinity threshold):")
    print(f"   Safe default: {tau_aff:.2f}")
    print(f"   Features passing: {n_features_aff}")
    
    # τ_cons: 0.40 (rilassato per catturare più feature interpretabili)
    tau_cons = 0.40
    ...
    print(f"\nTau_cons (mean consistency, labeling only):")
    print(f"   Relaxed threshold: {tau_cons:.2f}")
    ...
    if node_influences:
        tau_node_inf = np.percentile(node_influences, 85)  # Rilassato da p90
        tau_node_very_high = np.percentile(node_influences, 90)  # Rilassato da p95
        print(f"   τ_node_inf (node influence p85, relaxed): {tau_node_inf:.6f}")
        print(f"   τ_node_very_high (for <BOS> p90, relaxed): {tau_node_very_high:.6f}")
    else:
        print(f"   WARNING: Node influence non disponibile, uso solo logit_influence")
        tau_node_inf = 0
    ...
    # Soglia "influence altissima" per <BOS>: p90 (rilassato da p95)
    tau_inf_very_high = np.percentile(metrics_df['logit_influence'], 90)
    print(f"   τ_inf_very_high (logit influence for <BOS>): {tau_inf_very_high:.6f} (p90, relaxed)")
    ...
    if bos_pct > 50:
        print(f"   ⚠️ WARNING: BOS leakage >50%, dominance troppo alta")
    else:
        print(f"   ✅ BOS presence acceptable (<50%)")
    ...
    results = {
        ...
        'quality_checks': {
            ...
            'bos_leakage_ok': bool(bos_pct <= 50)
        }
    }
