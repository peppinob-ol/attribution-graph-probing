...
class BasicFeatureAnthropologist:
    ...
    def analyze_feature_personalities(self) -> Dict:
        ...
        for feature_key, records in feature_data.items():
            ...
            for r in records:
                if r.get('nuova_somma_sequenza'):
                    try:
                        ...
                        pass
                if r.get('nuova_max_label_span'):
                    try:
                        max_label_spans.append(float(r['nuova_max_label_span']))
                    except ValueError:
                        pass
                if r.get('nuova_somma_label_span'):
                    try:
                        sum_label_spans.append(float(r['nuova_somma_label_span']))
                    except ValueError:
                        pass
            ...
            # Calcola personalità con NUOVE METRICHE + PROBE ACTIVATIONS
            personality = {
                'layer': int(layer),
                ...
                # PROBE PROMPT ACTIVATIONS (FIX BUG)
                'nuova_max_label_span': max(max_label_spans) if max_label_spans else 0.0,
                'nuova_sum_label_span': sum(sum_label_spans) if sum_label_spans else 0.0,
                'nuova_mean_label_span': sum(max_label_spans) / len(max_label_spans) if max_label_spans else 0.0,
            }
    ...
    def identify_narrative_archetypes(self, personalities: Dict):
        ...
        Soglie riviste dopo analisi probe features:
        - Percentili abbassati da 75 a 70 (meno stringenti)
        - Semantic anchors: almeno 2 su 3 criteri semantici alti
        - Computational helpers: considerano anche node_influence e output_impact
        """
        ...
        output_impacts = [p['output_impact'] for p in personalities.values()]
        
        # SOGLIE RIVISTE: Percentile 70 invece di 75 (meno stringenti)
        high_mean_consistency = self._percentile(mean_consistencies, 70)
        high_label_affinity = self._percentile(label_affinities, 70)
        high_social = self._percentile(social_influences, 70)
        high_max_affinity = self._percentile(max_affinities, 70)
        high_node_influence = self._percentile([abs(ni) for ni in node_influences], 75)
        high_output_impact = self._percentile(output_impacts, 75)
        
        # Soglie assolute minime per semantic anchors
        MIN_SEMANTIC_CONSISTENCY = 0.6
        MIN_SEMANTIC_AFFINITY = 0.65
        
        print(f"📊 Soglie empiriche (riviste):")
        print(f"   Mean consistency alta: {high_mean_consistency:.3f} (min assoluto: {MIN_SEMANTIC_CONSISTENCY})")
        print(f"   Max affinity alta: {high_max_affinity:.3f} (min assoluto: {MIN_SEMANTIC_AFFINITY})")
        print(f"   Affinità label alta: {high_label_affinity:.3f}")
        print(f"   Influenza sociale alta: {high_social:.3f}")
        print(f"   Node influence alta: {high_node_influence:.3f}")
        print(f"   Output impact alto: {high_output_impact:.3f}")
        ...
        for feature_key, personality in personalities.items():
            mean_cons = personality['mean_consistency']
            max_aff = personality['max_affinity']
            label_aff = personality['label_affinity']
            node_inf = abs(personality.get('node_influence', 0))
            output_imp = personality['output_impact']
            
            # SEMANTIC ANCHORS: almeno 2 su 3 criteri semantici alti + soglie assolute
            semantic_criteria = [
            ...
            n_semantic_criteria = sum(semantic_criteria)
            
            if n_semantic_criteria >= 2:
            ...
            # STABLE CONTRIBUTORS: alta mean consistency ma non abbastanza per semantic anchor
            elif mean_cons > high_mean_consistency and max_aff > 0.4:
            ...
            elif max_aff > high_max_affinity and mean_cons < high_mean_consistency:
                archetypes['contextual_specialists'].append({
                    'feature_key': f"{feature_key[0]}_{feature_key[1]}" if isinstance(feature_key, tuple) else feature_key,
                    'personality': personality
                })
            
            # COMPUTATIONAL HELPERS: alto impatto causale ma bassa semantica
            # (Rivisto: considera node_influence e output_impact)
            ...
            # OUTLIERS: comportamento instabile o non classificabile
            else:
